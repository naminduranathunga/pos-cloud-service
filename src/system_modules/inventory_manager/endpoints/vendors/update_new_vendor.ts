...
export default async function update_vendor(req: Request, res: Response) {
    ...
    //validate request
    let { id, name, email, phone, address, is_active } = req.body as VendorBody;
    if (isNaN(id) || id < 1){
    ...
    // get the vendor
    let [vendors] = await conn.query<Array<any>>('SELECT * FROM vendors WHERE id = ?', [id]);
    if (vendors.length === 0){
    ...
    let query = `UPDATE vendors SET ${update_params.join(', ')} WHERE id = ?`;
