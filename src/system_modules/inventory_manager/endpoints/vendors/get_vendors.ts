 ...
 *  */
export default async function get_vendor_list(req: Request, res: Response) {
    ...
    if (id){
        ...
        let values = [];
        if (search){    
            query = `SELECT * FROM vendors WHERE name LIKE ? OR email LIKE ? OR phone LIKE ? OR address LIKE ? LIMIT ?, ?`;
            search = `%${search}%`;
            values = [search, search, search, search, (page - 1) * perPage, perPage];
        } else {
            query = `SELECT * FROM vendors LIMIT ?, ?`;
            values = [(page - 1) * perPage, perPage];
        }

        let [result] = await conn.query<Array<any>>(query, values);
        if (max_results){
            let [count] = await conn.query('SELECT COUNT(*) as count FROM vendors');
