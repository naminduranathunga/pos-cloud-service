...
/**
 * Endpoint api/v1/sys-company-manager/company-administration/migrate-company-dbs
 * Protected Route
 * This endpoint is used to migrate company databases to the latest version
 * @param req  
 * @param res 
 */
export default async function migrate_company_dbs(req: Request, res: Response){
    ...
    for(let i = 0; i < company_list.length; i++){
        ...
        try {
            ...
            await migrate_company_db(dbname);
            return_report.push({
                id: company._id,
                name: company.name,
                dbname: dbname,
